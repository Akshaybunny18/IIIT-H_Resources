...
if [ ${status3} -ne 0 ]; then
    ...
    run_test_3() {
        ...
        # Move input files to current directory as input1.txt and input2.txt
        mv "$input_file1" "input1.txt"
        mv "$input_file2" "input2.txt"
        ...
        MEMORY_LIMIT=$((4 * 1024))
        program_output=$(cat "output.txt" | tr -d '\n' | sed 's/[[:space:]]*$//')
        expected_output=$(echo -n "$expected_output" | tr -d '\n' | sed 's/[[:space:]]*$//')
        # Check if the program timed out
        if [ $exit_status -eq 124 ]; then
        ...
        # Move files back to original input file names
        mv "input1.txt" "$input_file1"
        mv "input2.txt" "$input_file2"
    }
    ...
    do
        ...
        input_file1="../$tests_folder/input1_$i.txt"
        input_file2="../$tests_folder/input2_$i.txt"
        output_file="../$tests_folder/output$i.txt"
        points=$((20 / num_tests))
        
        if [[ -f "$input_file1" && -f "$input_file2" && -f "$output_file" ]]; then
            run_test_3 "$input_file1" "$input_file2" "$output_file" "$points"
        else
            if [ ! -f "$input_file1" ]; then
                echo "Input file not found: $(realpath "$input_file1")"
                
            fi
            if [ ! -f "$input_file2" ]; then
                echo "Input file not found: $(realpath "$input_file2")"
            fi
            if [ ! -f "$output_file" ]; then
                echo "Output file not found: $(realpath "$output_file")"
            fi
