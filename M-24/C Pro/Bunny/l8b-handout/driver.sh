...
if [ ${status3} -ne 0 ]; then
    ...
    run_test_3() {
        ...
        if [ $exit_status -eq 124 ]; then
            ...
            # echo "$time_output" "$expected_output"
            # Check if the output matches the expected output
            if [ "$time_output" = "$expected_output" ]; then
    ...
    do
        ...
        input_file="$tests_folder/words$i.txt"
        output_file="$tests_folder/words$i-output.txt"
        points=$((20 / num_tests))
